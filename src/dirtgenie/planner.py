...
def generate_trip_plan(
    ...
    # Detect closed-loop tours to include special guidance (once)
    is_closed_loop = (
        start.lower().strip() == end.lower().strip()
        or (
            start.replace(",", "").replace(" ", "").lower() in end.replace(",", "").replace(" ", "").lower()
            or end.replace(",", "").replace(" ", "").lower() in start.replace(",", "").replace(" ", "").lower()
        )
    )
    ...
    if is_closed_loop:
        ...
        max_radius_km = int(avg_daily * (nights + 1) * 0.35)
        closed_loop_text = f"""
CLOSED-LOOP ROUTE GUIDANCE:
- Plan a true loop, not an out-and-back.
- Keep the route within roughly {max_radius_km}km of {start}.
- Ensure each overnight location can return to {start} within the remaining days at {daily_distance_value} km/day.
"""
    else:
        closed_loop_text = ""
...
def revise_trip_plan_with_feedback(
    ...
    if is_closed_loop:
        ...
        if "-" in daily_distance_value:
            ...
            avg_daily = int(daily_distance_value.split()[0]) if daily_distance_value else 50
        max_radius_km = int(avg_daily * (nights + 1) * 0.35)
        closed_loop_text = f"""CLOSED-LOOP ROUTE GUIDANCE:
- Plan a true loop, not an out-and-back.
- Keep the route within roughly {max_radius_km}km of {start}.
- Ensure each overnight location can return to {start} within the remaining days at {daily_distance_value} km/day."""
    else:
    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert bikepacking guide with access to current web information. You excel at incorporating user feedback to improve trip plans while maintaining quality and feasibility. Always search for current information when making changes to accommodations, routes, or activities.",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=4000,
